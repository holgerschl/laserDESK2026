...
```json
{
  ...
  "rif_execute_list_no": 1
}
```

- **`dxf_rif_list_upload`**: optional on **`POST /rtc/connect`** with **`mode: ethernet`**. Default **`true`** so DXF / scene jobs send **`R_DC_CONFIG_LIST`** (1), **`R_DC_GET_INPUT_POINTER`** (4), **`R_DC_LOAD_LIST_POS`** (6) with list = **`rif_execute_list_no`** and Pos from the pointer answer, then one UDP telegram per list command: **`R_LC_JUMP_XY_ABS`** / **`R_LC_MARK_XYZT_ABS`** per segment and **`R_LC_END_OF_LIST`**. Set **`false`** only for debugging ( **`load_dxf_job`** then fails fast on Ethernet with an empty list).
...
- **`rif_execute_list_no`**: optional; first word of **`R_DC_EXECUTE_LIST_POS`** (which list to run). If omitted, defaults to **`rif_config_list_mem1`**. Try **`rif_config_list_mem2`** if the board streams list commands into the secondary buffer but executes from list 2. Reported on **`GET /rtc/status`** as **`rif_execute_list_no`** when using Ethernet.

- **`port`**: defaults to **63750** if omitted (typical **UDP exclusive** port from RTC6conf; **confirm** with your RTC6 package / `eth_get_port_numbers` — see `docs/rtc/bring-up-checklist-phase-c.md`).
...
**Load / run / stop:** After a successful connect **`R_DC_GET_STATUS`**, Ethernet probes **`R_DC_GET_RTC_VERSION` (142)** / **`R_DC_GET_BIOS_VERSION` (143)** when **`expected_package_tag` / `expected_bios_eth_tag`** are empty (SCANLAB `RIF_test` RI path). DXF load (with list upload) uses **`R_DC_SET_START_LIST_POS` (3)**, then **`R_DC_LOAD_LIST_POS` (6)**, after **`R_DC_GET_INPUT_POINTER`** (see `docs/rtc/rif-command-coverage.md`). **Run** sends **`R_DC_SET_MAX_COUNT` (11)** (query `repeat_count` on `POST …/run`, default 1) then **`R_DC_EXECUTE_LIST_POS` (15)** with list = **`rif_execute_list_no`** (default **`rif_config_list_mem1`**) and position = input pointer captured at DXF load. **`R_DC_STOP_EXECUTION` (16)** stops execution. **REST `connection_state`:** on Ethernet, a successful **`GET /rtc/status`** may move **`running` → `loaded`** when **`R_DC_GET_STATUS`** shows no list-execution busy bits **only if** that run used **`repeat_count` = 1**; for **`repeat_count` > 1**, use **`POST /rtc/stop`** to return to **`loaded`** (Ch.10 **`set_max_counts`** maps to External Starts, not confirmed here as list-body repeat count). Optional **DXF list build** adds **`R_DC_CONFIG_LIST` (1)** and **`R_LC_*`** per package `telegrams.h`.
