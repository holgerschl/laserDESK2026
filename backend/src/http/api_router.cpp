...
int BackendSession::handle_post_rtc_connect(const nlohmann::json& body, nlohmann::json& err_out) {
  ...
  if (mode_s == "ethernet") {
    ...
    // Semantic mm/s at connect (image plane); Ethernet converts to RIF bits/ms using K (bits/mm) at start.
    if (body.contains("rif_jump_speed_mm_s") && body["rif_jump_speed_mm_s"].is_number())
      cfg.rif_jump_speed_mm_s = body["rif_jump_speed_mm_s"].get<double>();
    else if (body.contains("rif_jump_speed_m_s") && body["rif_jump_speed_m_s"].is_number())
      cfg.rif_jump_speed_mm_s = body["rif_jump_speed_m_s"].get<double>() * 1000.0;
    if (body.contains("rif_mark_speed_mm_s") && body["rif_mark_speed_mm_s"].is_number())
      cfg.rif_mark_speed_mm_s = body["rif_mark_speed_mm_s"].get<double>();
    else if (body.contains("rif_mark_speed_m_s") && body["rif_mark_speed_m_s"].is_number())
      cfg.rif_mark_speed_mm_s = body["rif_mark_speed_m_s"].get<double>() * 1000.0;
