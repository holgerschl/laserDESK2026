...
- **Scene editor:** `/editor` — Konva canvas (`SceneEditor.svelte`), job tree + **parameters** beside canvas (wide shell); laser **presets** (`EntityLaserPanel`, `sceneV1`); **Shift+click** multi-select (`scene/selection.ts`).
