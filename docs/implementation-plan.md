...
| H.4 | **Primitives (vertical slice):** place **line** and **rectangle**; **select**, **move**, **delete**; single **layer**; **pan/zoom** viewport. **Done:** `SceneEditor.svelte` — Konva `Group` (`viewPanX/Y`, `viewZoom`); wheel / Space+drag / middle-mouse pan; **mm SVG**: stage-sized `<svg>` + inner `<g matrix(…)>` matching Konva (not CSS on root `<svg>`); **Reset view**; `stageToWorldMm`; **line** / **rect** tools: **click-drag-release** with **dashed preview** (rubber-band) in world mm; `window` pointer move/up for release off-canvas; shapes non-listening while placing; shapes **draggable when selected** (move + transformer). |
...
- **Phase H:** **Vector scene editor** (Konva/Fabric per §4 Phase H) — place/manipulate geometry, submit scene job, run on mock; **pan/zoom** per **F-09** / H.4; **line/rect drag placement + preview** per **F-10**; **laser presets / per-entity laser** per **F-13**; **multi-select** per **F-11**; **X-04** / **X-05** when implemented.
