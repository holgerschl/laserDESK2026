...
| H.4 | **Primitives (vertical slice):** place **line**, **rectangle**, **arc**, **text**; **select**, **move**, **delete**; single **layer**; **pan/zoom** viewport. **Done:** `SceneEditor.svelte` — Konva `Group` (`viewPanX/Y`, `viewZoom`); wheel / Space+drag / middle-mouse pan; **mm SVG**: stage-sized `<svg>` + inner `<g matrix(…)>` matching Konva (not CSS on root `<svg>`); **Reset view**; `stageToWorldMm`; **line** / **rect** / **arc** (drag center→radius + preview) / **text** (click + prompt); **dashed preview** where applicable; `window` pointer move/up for release off-canvas; shapes non-listening while placing; shapes **draggable when selected** (move + transformer; **arc** move-only, no transformer). |
