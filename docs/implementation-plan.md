...
| H.4 | **Primitives (vertical slice):** place **line** and **rectangle**; **select**, **move**, **delete**; single **layer**; **pan/zoom** viewport. **Done:** `SceneEditor.svelte` — Konva `Group` (`viewPanX/Y`, `viewZoom`); wheel / Space+drag / middle-mouse pan; **mm SVG** overlay: same affine transform as Konva in **screen pixels** (`matrix(zoom,0,0,zoom,panX,panY)` CSS on the SVG with `width`/`height` = stage, `transform-origin: 0 0`); **Reset view**; `stageToWorldMm` for pointer→world. Shapes **draggable when selected** in select mode (move + transformer). |
