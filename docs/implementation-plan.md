...
**Status:** **In progress (MVP slice + G.4 mapper done):** ASCII DXF **LINE** parser + REST `/api/v1/jobs/dxf` + mock/ethernet **`load_dxf_job`**. **G.4:** `RtcJobPlan`, `dxf_rif_list_mapper` (2D Z = `NO_Z_MOVE`), ethernet path **`R_DC_CONFIG_LIST`** → **`R_DC_GET_INPUT_POINTER`** → **`R_DC_LOAD_LIST_POS`** → `R_LC_*` stream when **`dxf_rif_list_upload`** (see [`docs/rtc/rif-command-coverage.md`](rtc/rif-command-coverage.md)). Frontend **`/dxf`** + Playwright **`dxf-demo.spec.ts`**. **Deferred:** richer list semantics (speeds, laser on/off); extend parser beyond LINE as needed.
...
**Status:** **In MVP catalog** ([`docs/requirements/mvp-feature-katalog.md`](requirements/mvp-feature-katalog.md) §2: **B-10…B-12**, **F-08…F-13**, **X-04…X-05**; promotion note §5). **Implementation** of tasks H.1–H.9 is **in progress** until closed out with tests per catalog §4. **H.4 (partial):** **pan/zoom** and **mm SVG overlay** (fixed `width`/`height` = stage, inner `<g>` `matrix` matching Konva viewport — avoid CSS `transform` on root `<svg>`, which can hide the overlay) are implemented (**F-09**); **laser presets and per-entity laser** are implemented (**F-13** / **B-10** metadata): `scene_v1` carries `laser_groups`, `default_laser_group_id`, per-entity `laser_group_id` and optional `laser` override (`frontend/src/lib/scene/sceneV1.ts`); entity types **`line`**, **`rect`**, **`arc`**, **`text`**; optional **`entity_label`** for stable job-tree naming; optional **`job_group_id` / `job_group_label`** for **job tree** folder grouping (UI-only; backend ignores on geometry path). **Backend** `scene_v1_to_parse_result` tessellates **`arc`** and maps **`text`** to a box outline (+ diagonal) for LINE output (`backend/src/rtc/job/scene_v1.cpp`). UI: **`EntityLaserPanel.svelte`** — **Preset parameters** (dropdown + **`LaserPropertiesPanel`**) always visible without canvas selection; **Preset names** (editable display names) in a **collapsible** section **collapsed by default**; preset CRUD; bulk laser apply when multi-selected. **`SceneJobTree.svelte`** — collapsible rows; **Shift+click** range select; **Group** / **Ungroup**; nested group rows; **↑/↓** on a member row (single selection) = **one entity** step + `sanitizeFragmentedJobGroups()`; **↑/↓** on folder when **full group** selected = **whole block**. **`selection.ts`** — shared multi-select logic (**F-11**). **`SceneEditor.svelte`:** **line / rect / arc / text** tools (**SVG** icons for line/rect/arc, **Text** label); multi-select **translate together**; Konva **Transformer** on selected nodes (**arc** as **Group** + invisible guide **circle**; **orange end handle** for **sweep**; **keepRatio** when exactly one arc is selected). No separate `LaserGroupsPanel` — presets are edited in the parameter column. **Wide** editor shell on `/editor`. Remaining H.4 items include layer panel and polish. Delivers **interactive placement and manipulation** of geometric entities in the browser using a **canvas-oriented library** (decision: **Konva** or **Fabric.js** or equivalent; **MIT license**, bundle impact documented). The **canonical contract** remains a **versioned scene JSON** owned by laserDESK 2026; the library holds transient interaction state only and must **sync to/from** that model.
...
| H.4 | **Primitives (vertical slice):** place **line**, **rectangle**, **arc**, **text**; **select**, **move**, **delete**; single **layer**; **pan/zoom** viewport. **Done:** `SceneEditor.svelte` — Konva viewport `Group` (`viewPanX/Y`, `viewZoom`); wheel / Space+drag / middle-mouse pan; **mm SVG**: stage-sized `<svg>` + inner `<g matrix(…)>` matching Konva (not CSS on root `<svg>`); **Reset view**; `stageToWorldMm`; **toolbar:** line/rect/arc as **SVG icons**, **Text** as text label; **line** / **rect** / **arc** (drag center→radius + preview) / **text** (click + prompt, default **`DEFAULT_TEXT_HEIGHT_MM`**); **dashed preview** where applicable; `window` pointer move/up for release off-canvas; shapes non-listening while placing; shapes **draggable when selected** (move + transformer); **arc** = entity **Group** + local polyline + guide **circle** + **sweep** handle + Transformer (**keepRatio** for single arc). |
| H.5 | **Manipulators:** rotation / scale handles (library features or thin wrapper). **Multi-select:** **Shift+click** range in job list + canvas (`selection.ts`); highlight multiple; **Konva `Transformer`** with **multiple nodes** so rotate/resize applies to the **whole selection**; multi-drag translation keeps peers in sync. |
...
| Scene schema | TypeScript types + `buildSceneV1()` in [`frontend/src/lib/scene/sceneV1.ts`](../frontend/src/lib/scene/sceneV1.ts); arc helpers (`arcMm.ts`); optional JSON Schema / OpenAPI later |
| Editor UI | `SceneEditor.svelte` (Konva + pan/zoom + mm overlay); `EntityLaserPanel.svelte`, `SceneJobTree.svelte` (job **groups**: **↑/↓** on a row = one entity; on folder when full group selected = whole block; `sanitizeFragmentedJobGroups` in `sceneV1.ts`), `scene/selection.ts`; route [`frontend/src/routes/editor/`](../frontend/src/routes/editor/) |
| API | `backend` — scene job endpoint + validation; [`scene_v1.cpp`](../backend/src/rtc/job/scene_v1.cpp) flattens `line` / `rect` / `arc` / `text` to LINE list; mapper next to existing DXF → RIF path |
...
- **Phase H:** **Vector scene editor** (Konva/Fabric per §4 Phase H) — place/manipulate geometry, submit scene job, run on mock; **pan/zoom** per **F-09** / H.4; **line / rect / arc / text** tools per **F-10**; **laser presets / per-entity laser** per **F-13**; **multi-select** per **F-11**; **X-04** / **X-05** when implemented.
